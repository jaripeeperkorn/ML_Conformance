...
def get_dist_vars(filenamelog, filenamemodel,  modellogsize=None, max_occ=3, bigram=False, vector_size = None, window_size = 2):
    ...
    def wmd_distmatrix(eventlog, modellog, where_zero, where_zero2):
    ...
    def wmd_distmatrix_when_no_overlap(eventlog, modellog):
        distances = np.zeros((len(modellog),len(eventlog)))
        for i in range(len(modellog)):
            for j in range(len(eventlog)):
                distances[i][j] = WMD[i,j]
    ...
    if len(where_zero) == 0 or len(where_zero2) == 0:
        wmd_disM = wmd_distmatrix_when_no_overlap(event_log_variants, model_log_variants)
    else:
        wmd_disM = wmd_distmatrix(event_log_variants, model_log_variants, where_zero, where_zero2)

    print("got matrix")

    fitness_wmd, precision_wmd = get_distances_vars(wmd_disM, event_log_counts, model_log_counts, event_log_size, model_log_size)
    
    return(fitness_wmd, precision_wmd)  
...
def get_dist_full(filenamelog, filenamemodel,  modellogsize=None, max_occ=3, bigram=False, vector_size = None, window_size = 2):
    ...
    WMD = WmDistance(model.wv, model_log, event_log)
